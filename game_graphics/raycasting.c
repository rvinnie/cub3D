...
void	find_first_horisontal_wall(t_ray *s_ray, t_player *s_player)
{
	...
	{
		first_h_y = floor(y_pos / PXL_SIZE) * PXL_SIZE - 1; // A.y = rounded_down(Py/64) * (64) - 1
		first_h_x = x_pos + (y_pos - first_h_y) / tan(s_ray->alpha); // 4 + sqrt(3)/3
	}
	else														 // If the ray is facing down
	{
		first_h_y = floor(y_pos / PXL_SIZE) * PXL_SIZE + PXL_SIZE;	// A.y = rounded_down(Py/64) * (64) + 64;
...
void	find_first_vertical_wall(t_ray *s_ray, t_player *s_player)
{
	...
	if (s_ray->alpha >= M_PI / 2 && s_ray->alpha <= 3 * M_PI / 2)
	{
		first_h_x = floor(x_pos / PXL_SIZE) * PXL_SIZE - 1;
		first_h_y = y_pos + (x_pos - first_h_x) * tan(s_ray->alpha); //A.y = Py + (Px-A.x)*tan(ALPHA);
	}
	else
	{
		first_h_x = floor(x_pos / PXL_SIZE) * PXL_SIZE + PXL_SIZE;
